...
module Mesos.V1.Protos.MachineInfo (MachineInfo(..)) where
import Prelude ((+), (/))
...
data MachineInfo = MachineInfo{id :: (Protos.MachineID), mode :: (P'.Maybe Protos.MachineInfo.Mode),
                               unavailability :: (P'.Maybe Protos.Unavailability)}
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             16 -> Prelude'.fmap (\ !new'Field -> old'Self{mode = Prelude'.Just new'Field}) (P'.wireGet 14)
             26 -> Prelude'.fmap
...
instance P'.TextMsg MachineInfo where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'id, parse'mode, parse'unavailability]) P'.spaces
