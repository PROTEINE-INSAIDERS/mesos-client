...
module Mesos.V1.Protos.Event.LaunchGroup (LaunchGroup(..)) where
import Prelude ((+), (/))
...
data LaunchGroup = LaunchGroup{task_group :: !(Protos.TaskGroupInfo)}
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{task_group = P'.mergeAppend (task_group old'Self) (new'Field)})
                    (P'.wireGet 11)
...
instance P'.ReflectDescriptor LaunchGroup where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".mesos.v1.executor.Event.LaunchGroup\", haskellPrefix = [MName \"Mesos\",MName \"V1\"], parentModule = [MName \"Protos\",MName \"Event\"], baseName = MName \"LaunchGroup\"}, descFilePath = [\"Mesos\",\"V1\",\"Protos\",\"Event\",\"LaunchGroup.hs\"], isGroup = False, fields = fromList [FieldInfo {fieldName = ProtoFName {protobufName' = FIName \".mesos.v1.executor.Event.LaunchGroup.task_group\", haskellPrefix' = [MName \"Mesos\",MName \"V1\"], parentModule' = [MName \"Protos\",MName \"Event\",MName \"LaunchGroup\"], baseName' = FName \"task_group\", baseNamePrefix' = \"\"}, fieldNumber = FieldId {getFieldId = 1}, wireTag = WireTag {getWireTag = 10}, packedTag = Nothing, wireTagLength = 1, isPacked = False, isRequired = True, canRepeat = False, mightPack = False, typeCode = FieldType {getFieldType = 11}, typeName = Just (ProtoName {protobufName = FIName \".mesos.v1.TaskGroupInfo\", haskellPrefix = [MName \"Mesos\",MName \"V1\"], parentModule = [MName \"Protos\"], baseName = MName \"TaskGroupInfo\"}), hsRawDefault = Nothing, hsDefault = Nothing}], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"
...
instance P'.TextMsg LaunchGroup where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'task_group]) P'.spaces
