...
import Mesos.V1.Protos.FrameworkInfo.Capability (Capability(..))
import Mesos.V1.Protos.FrameworkInfo.Capability.Type (Type(..))
...
my_labels = Labels { labels = Label { key = uFromString "mesos-client", value = Just $ uFromString "prototype" }  <| Empty }

fin = FrameworkInfo
        { user = uFromString "schernichkin"
...
main = do
    ...
    (res :: Master.Response) <- call endpoint M.jsonEncoder M.jsonSink Master.getHealth
    let s =  Scheduler.Subscribe 
         { Scheduler.framework_info = fin
