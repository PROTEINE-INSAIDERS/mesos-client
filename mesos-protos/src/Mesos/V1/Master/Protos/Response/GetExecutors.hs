...
module Mesos.V1.Master.Protos.Response.GetExecutors (GetExecutors(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "GetExecutors"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{executors = executors, orphan_executors = orphan_executors})
