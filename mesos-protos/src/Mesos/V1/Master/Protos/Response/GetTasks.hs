...
module Mesos.V1.Master.Protos.Response.GetTasks (GetTasks(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "GetTasks"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{pending_tasks = pending_tasks, tasks = tasks, unreachable_tasks = unreachable_tasks,
                           completed_tasks = completed_tasks, orphan_tasks = orphan_tasks})
