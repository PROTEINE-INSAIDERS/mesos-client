...
module Mesos.V1.Master.Protos.Response.GetMaster (GetMaster(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "GetMaster"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{master_info = master_info, start_time = start_time, elected_time = elected_time})
