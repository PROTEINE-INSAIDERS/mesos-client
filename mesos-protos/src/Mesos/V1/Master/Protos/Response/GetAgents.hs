...
instance P'.ToJSON GetAgents where
  toJSON msg
   = P'.objectNoEmpty
      ([("agents", P'.toJSON (Prelude'.fmap P'.toJSON (agents msg))),
        ("recovered_agents", P'.toJSON (Prelude'.fmap P'.toJSON (recovered_agents msg)))]
        ++ Prelude'.concat [])

instance P'.FromJSON GetAgents where
  parseJSON
   = P'.withObject "GetAgents"
      (\ o ->
        do
          agents <- Prelude'.fmap (Prelude'.maybe Prelude'.mempty Prelude'.id)
