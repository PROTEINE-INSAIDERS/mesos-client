...
module Mesos.V1.Master.Protos.Response.GetMaintenanceStatus (GetMaintenanceStatus(..), status) where
import Prelude ((+), (/), (++), (.))
...
data GetMaintenanceStatus = GetMaintenanceStatus{_status :: !(Protos.ClusterStatus)}
                            deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''GetMaintenanceStatus
