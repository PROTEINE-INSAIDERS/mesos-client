...
module Mesos.V1.Master.Protos.Response.GetState (GetState(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "GetState"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{get_tasks = get_tasks, get_executors = get_executors, get_frameworks = get_frameworks,
