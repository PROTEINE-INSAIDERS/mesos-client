...
module Mesos.V1.Master.Protos.Response.GetAgents.Agent.ResourceProvider
       (ResourceProvider(..), resource_provider_info, total_resources) where
import Prelude ((+), (/), (++), (.))
...
data ResourceProvider = ResourceProvider{_resource_provider_info :: !(Protos.ResourceProviderInfo),
                                         _total_resources :: !(P'.Seq Protos.Resource)}
                        deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''ResourceProvider
   ...
   = P'.withObject "ResourceProvider"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_resource_provider_info = resource_provider_info, _total_resources = total_resources})
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap
                    (\ !new'Field ->
                      old'Self{_resource_provider_info = P'.mergeAppend (_resource_provider_info old'Self) (new'Field)})
                    (P'.wireGet 11)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_total_resources = P'.append (_total_resources old'Self) new'Field})
                    (P'.wireGet 11)
...
instance P'.TextMsg ResourceProvider where
  textPut msg
   = do
       P'.tellT "resource_provider_info" (_resource_provider_info msg)
       P'.tellT "total_resources" (_total_resources msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_resource_provider_info, parse'_total_resources]) P'.spaces
       Prelude'.return (Prelude'.foldl (\ v f -> f v) P'.defaultValue mods)
    where
        parse'_resource_provider_info
         = P'.try
            (do
               v <- P'.getT "resource_provider_info"
               Prelude'.return (\ o -> o{_resource_provider_info = v}))
