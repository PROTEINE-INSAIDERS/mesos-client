...
module Mesos.V1.Master.Protos.Call.MarkAgentGone (MarkAgentGone(..), agent_id) where
import Prelude ((+), (/), (++), (.))
...
data MarkAgentGone = MarkAgentGone{_agent_id :: !(Protos.AgentID)}
                     deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''MarkAgentGone

instance P'.ToJSON MarkAgentGone where
  toJSON msg = P'.objectNoEmpty ([("agent_id", P'.toJSON (agent_id msg))] ++ Prelude'.concat [])

instance P'.FromJSON MarkAgentGone where
  parseJSON
   = P'.withObject "MarkAgentGone"
      (\ o ->
        do
          agent_id <- P'.explicitParseField P'.parseJSON o "agent_id"
          Prelude'.return P'.defaultValue{_agent_id = agent_id})
...
instance P'.TextMsg MarkAgentGone where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_agent_id]) P'.spaces
