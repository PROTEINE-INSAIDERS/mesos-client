...
module Mesos.V1.Master.Protos.Call.UpdateQuota (UpdateQuota(..), quota_requests) where
import Prelude ((+), (/), (++), (.))
...
data UpdateQuota = UpdateQuota{_quota_requests :: !(P'.Seq Protos.QuotaRequest)}
                   deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''UpdateQuota
   ...
   = P'.withObject "UpdateQuota"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_quota_requests = quota_requests})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_quota_requests = P'.append (_quota_requests old'Self) new'Field})
                    (P'.wireGet 11)
...
instance P'.TextMsg UpdateQuota where
  textPut msg
   = do
       P'.tellT "quota_requests" (_quota_requests msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_quota_requests]) P'.spaces
       Prelude'.return (Prelude'.foldl (\ v f -> f v) P'.defaultValue mods)
    where
        parse'_quota_requests
         = P'.try
            (do
               v <- P'.getT "quota_requests"
               Prelude'.return (\ o -> o{_quota_requests = P'.append (_quota_requests o) v}))