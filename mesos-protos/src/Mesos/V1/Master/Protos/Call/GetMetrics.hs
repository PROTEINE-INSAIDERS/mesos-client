...
module Mesos.V1.Master.Protos.Call.GetMetrics (GetMetrics(..), timeout) where
import Prelude ((+), (/), (++), (.))
...
data GetMetrics = GetMetrics{_timeout :: !(P'.Maybe Protos.DurationInfo)}
                  deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''GetMetrics

instance P'.ToJSON GetMetrics where
  toJSON msg = P'.objectNoEmpty ([("timeout", P'.toJSON (Prelude'.fmap P'.toJSON (timeout msg)))] ++ Prelude'.concat [])
...
instance P'.FromJSON GetMetrics where
  parseJSON
   = P'.withObject "GetMetrics"
      (\ o ->
        do
          timeout <- P'.explicitParseFieldMaybe P'.parseJSON o "timeout"
          Prelude'.return P'.defaultValue{_timeout = timeout})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_timeout = P'.mergeAppend (_timeout old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg GetMetrics where
  textPut msg
   = do
       P'.tellT "timeout" (_timeout msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_timeout]) P'.spaces
