...
module Mesos.V1.Master.Protos.Call.StartMaintenance (StartMaintenance(..), machines) where
import Prelude ((+), (/), (++), (.))
...
data StartMaintenance = StartMaintenance{_machines :: !(P'.Seq Protos.MachineID)}
                        deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''StartMaintenance

instance P'.ToJSON StartMaintenance where
  toJSON msg = P'.objectNoEmpty ([("machines", P'.toJSON (Prelude'.fmap P'.toJSON (machines msg)))] ++ Prelude'.concat [])

instance P'.FromJSON StartMaintenance where
  parseJSON
   = P'.withObject "StartMaintenance"
      (\ o ->
        do
