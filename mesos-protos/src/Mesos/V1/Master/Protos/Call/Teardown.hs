...
module Mesos.V1.Master.Protos.Call.Teardown (Teardown(..), framework_id) where
import Prelude ((+), (/), (++), (.))
...
data Teardown = Teardown{_framework_id :: !(Protos.FrameworkID)}
                deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Teardown

instance P'.ToJSON Teardown where
  toJSON msg = P'.objectNoEmpty ([("framework_id", P'.toJSON (framework_id msg))] ++ Prelude'.concat [])

instance P'.FromJSON Teardown where
  parseJSON
   = P'.withObject "Teardown"
      (\ o ->
        do
          framework_id <- P'.explicitParseField P'.parseJSON o "framework_id"
          Prelude'.return P'.defaultValue{_framework_id = framework_id})
...
instance P'.TextMsg Teardown where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_framework_id]) P'.spaces
