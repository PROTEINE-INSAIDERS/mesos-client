...
module Mesos.V1.Master.Protos.Call.SetQuota (SetQuota(..), quota_request) where
import Prelude ((+), (/), (++), (.))
...
data SetQuota = SetQuota{_quota_request :: !(Protos.QuotaRequest)}
                deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''SetQuota
   ...
   = P'.withObject "SetQuota"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_quota_request = quota_request})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_quota_request = P'.mergeAppend (_quota_request old'Self) (new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg SetQuota where
  textPut msg
   = do
       P'.tellT "quota_request" (_quota_request msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_quota_request]) P'.spaces
       Prelude'.return (Prelude'.foldl (\ v f -> f v) P'.defaultValue mods)
    where
        parse'_quota_request
         = P'.try
            (do
               v <- P'.getT "quota_request"
               Prelude'.return (\ o -> o{_quota_request = v}))