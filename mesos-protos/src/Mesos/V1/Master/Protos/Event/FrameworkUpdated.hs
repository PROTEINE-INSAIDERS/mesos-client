...
module Mesos.V1.Master.Protos.Event.FrameworkUpdated (FrameworkUpdated(..), framework) where
import Prelude ((+), (/), (++), (.))
...
data FrameworkUpdated = FrameworkUpdated{_framework :: !(Protos.Response.GetFrameworks.Framework)}
                        deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''FrameworkUpdated

instance P'.ToJSON FrameworkUpdated where
  toJSON msg = P'.objectNoEmpty ([("framework", P'.toJSON (framework msg))] ++ Prelude'.concat [])

instance P'.FromJSON FrameworkUpdated where
  parseJSON
   = P'.withObject "FrameworkUpdated"
      (\ o ->
        do
          framework <- P'.explicitParseField P'.parseJSON o "framework"
          Prelude'.return P'.defaultValue{_framework = framework})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_framework = P'.mergeAppend (_framework old'Self) (new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg FrameworkUpdated where
  textPut msg
   = do
       P'.tellT "framework" (_framework msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_framework]) P'.spaces
    ...
    where
        parse'_framework
         = P'.try
            (do
               v <- P'.getT "framework"
               Prelude'.return (\ o -> o{_framework = v}))