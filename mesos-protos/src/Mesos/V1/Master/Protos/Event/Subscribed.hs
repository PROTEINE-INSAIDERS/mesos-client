...
module Mesos.V1.Master.Protos.Event.Subscribed (Subscribed(..)) where
import Prelude ((+), (/), (++), (.))
...
data Subscribed = Subscribed{get_state :: !(P'.Maybe Protos.Response.GetState), heartbeat_interval_seconds :: !(P'.Maybe P'.Double)}
   ...
   = P'.withObject "Subscribed"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{get_state = get_state, heartbeat_interval_seconds = heartbeat_interval_seconds})
...
instance P'.TextMsg Subscribed where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'get_state, parse'heartbeat_interval_seconds]) P'.spaces
