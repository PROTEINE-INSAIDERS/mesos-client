...
module Mesos.V1.Protos.Volume (Volume(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "Volume"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{mode = mode, container_path = container_path, host_path = host_path, image = image, source = source})
