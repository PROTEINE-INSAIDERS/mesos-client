...
module Mesos.V1.Protos.DeviceAccess (DeviceAccess(..), device, access) where
import Prelude ((+), (/), (++), (.))
...
data DeviceAccess = DeviceAccess{_device :: !(Protos.Device), _access :: !(Protos.DeviceAccess.Access)}
                    deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''DeviceAccess
   ...
   = P'.withObject "DeviceAccess"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_device = device, _access = access})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_device = P'.mergeAppend (_device old'Self) (new'Field)}) (P'.wireGet 11)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_access = P'.mergeAppend (_access old'Self) (new'Field)}) (P'.wireGet 11)
...
instance P'.TextMsg DeviceAccess where
  textPut msg
   = do
       ...
       P'.tellT "access" (_access msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_device, parse'_access]) P'.spaces
