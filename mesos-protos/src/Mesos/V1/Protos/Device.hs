...
module Mesos.V1.Protos.Device (Device(..), path, number) where
import Prelude ((+), (/), (++), (.))
...
data Device = Device{_path :: !(P'.Maybe P'.Utf8), _number :: !(P'.Maybe Protos.Device.Number)}
              deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Device
   ...
   = P'.withObject "Device"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_path = path, _number = number})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_path = Prelude'.Just new'Field}) (P'.wireGet 9)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_number = P'.mergeAppend (_number old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg Device where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_path, parse'_number]) P'.spaces
