...
instance P'.ToJSON CommandInfo where
  toJSON msg
   = P'.objectNoEmpty
      ([("uris", P'.toJSON (Prelude'.fmap P'.toJSON (uris msg))),
        ("environment", P'.toJSON (Prelude'.fmap P'.toJSON (environment msg))),
        ("shell", P'.toJSON (Prelude'.fmap P'.toJSON (shell msg))), ("value", P'.toJSON (Prelude'.fmap P'.toJSON (value msg))),
        ("arguments", P'.toJSON (Prelude'.fmap P'.toJSON (arguments msg))),
        ("user", P'.toJSON (Prelude'.fmap P'.toJSON (user msg)))]
        ++ Prelude'.concat [])

instance P'.FromJSON CommandInfo where
  parseJSON
   = P'.withObject "CommandInfo"
      (\ o ->
        do
          uris <- Prelude'.fmap (Prelude'.maybe Prelude'.mempty Prelude'.id)
                   (P'.explicitParseFieldMaybe (Prelude'.mapM P'.parseJSON P'.<=< P'.parseJSON) o "uris")
          environment <- P'.explicitParseFieldMaybe P'.parseJSON o "environment"
          shell <- do
                     tmp <- P'.explicitParseFieldMaybe P'.parseJSONBool o "shell"
          ...
          arguments <- Prelude'.fmap (Prelude'.maybe Prelude'.mempty Prelude'.id)
