...
module Mesos.V1.Protos.DeviceAccess.Access (Access(..), read, write, mknod) where
import Prelude ((+), (/), (++), (.))
...
data Access = Access{_read :: !(P'.Maybe P'.Bool), _write :: !(P'.Maybe P'.Bool), _mknod :: !(P'.Maybe P'.Bool)}
              deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Access
   ...
   = P'.withObject "Access"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_read = read, _write = write, _mknod = mknod})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             8 -> Prelude'.fmap (\ !new'Field -> old'Self{_read = Prelude'.Just new'Field}) (P'.wireGet 8)
             16 -> Prelude'.fmap (\ !new'Field -> old'Self{_write = Prelude'.Just new'Field}) (P'.wireGet 8)
             24 -> Prelude'.fmap (\ !new'Field -> old'Self{_mknod = Prelude'.Just new'Field}) (P'.wireGet 8)
...
instance P'.TextMsg Access where
  textPut msg
   = do
       P'.tellT "read" (_read msg)
       P'.tellT "write" (_write msg)
       P'.tellT "mknod" (_mknod msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_read, parse'_write, parse'_mknod]) P'.spaces
    ...
    where
        parse'_read
         = P'.try
            (do
               v <- P'.getT "read"
               Prelude'.return (\ o -> o{_read = v}))
        parse'_write
         = P'.try
            (do
               v <- P'.getT "write"
               Prelude'.return (\ o -> o{_write = v}))
        parse'_mknod
         = P'.try
            (do
               v <- P'.getT "mknod"
               Prelude'.return (\ o -> o{_mknod = v}))