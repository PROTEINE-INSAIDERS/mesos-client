...
module Mesos.V1.Protos.Device.Number (Number(..), major_number, minor_number) where
import Prelude ((+), (/), (++), (.))
...
data Number = Number{_major_number :: !(P'.Word64), _minor_number :: !(P'.Word64)}
              deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Number
   ...
   = P'.withObject "Number"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_major_number = major_number, _minor_number = minor_number})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             8 -> Prelude'.fmap (\ !new'Field -> old'Self{_major_number = new'Field}) (P'.wireGet 4)
             16 -> Prelude'.fmap (\ !new'Field -> old'Self{_minor_number = new'Field}) (P'.wireGet 4)
...
instance P'.TextMsg Number where
  textPut msg
   = do
       P'.tellT "major_number" (_major_number msg)
       P'.tellT "minor_number" (_minor_number msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_major_number, parse'_minor_number]) P'.spaces
        ...
        parse'_major_number
         = P'.try
            (do
               ...
               Prelude'.return (\ o -> o{_major_number = v}))
        parse'_minor_number
         = P'.try
            (do
               v <- P'.getT "minor_number"
               Prelude'.return (\ o -> o{_minor_number = v}))