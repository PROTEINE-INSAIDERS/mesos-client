...
instance P'.FromJSON Type where
  parseJSON
   = P'.withText "Mesos.V1.Protos.Secret.Type.Type"
      (\ msg' ->
        case msg' of
          "UNKNOWN" -> Prelude'.return UNKNOWN
          "REFERENCE" -> Prelude'.return REFERENCE
          "VALUE" -> Prelude'.return VALUE
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++ " for enum Mesos.V1.Protos.Secret.Type.Type"))