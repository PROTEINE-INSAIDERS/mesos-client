...
module Mesos.V1.Protos.KillPolicy (KillPolicy(..), grace_period) where
import Prelude ((+), (/), (++), (.))
...
data KillPolicy = KillPolicy{_grace_period :: !(P'.Maybe Protos.DurationInfo)}
                  deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''KillPolicy
   ...
   = P'.withObject "KillPolicy"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_grace_period = grace_period})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap
                    (\ !new'Field -> old'Self{_grace_period = P'.mergeAppend (_grace_period old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg KillPolicy where
  textPut msg
   = do
       P'.tellT "grace_period" (_grace_period msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_grace_period]) P'.spaces
        ...
        parse'_grace_period
         = P'.try
            (do
               ...
               Prelude'.return (\ o -> o{_grace_period = v}))