...
module Mesos.V1.Protos.Label (Label(..)) where
import Prelude ((+), (/), (++), (.))
...
data Label = Label{key :: !(P'.Utf8), value :: !(P'.Maybe P'.Utf8)}
   ...
   = P'.withObject "Label"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{key = key, value = value})
