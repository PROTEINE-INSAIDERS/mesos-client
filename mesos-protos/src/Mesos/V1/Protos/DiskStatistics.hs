...
module Mesos.V1.Protos.DiskStatistics (DiskStatistics(..), source, persistence, limit_bytes, used_bytes) where
import Prelude ((+), (/), (++), (.))
...
data DiskStatistics = DiskStatistics{_source :: !(P'.Maybe Protos.Resource.DiskInfo.Source),
                                     _persistence :: !(P'.Maybe Protos.Resource.DiskInfo.Persistence),
                                     _limit_bytes :: !(P'.Maybe P'.Word64), _used_bytes :: !(P'.Maybe P'.Word64)}
                      deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''DiskStatistics
   ...
   = P'.withObject "DiskStatistics"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{_source = source, _persistence = persistence, _limit_bytes = limit_bytes, _used_bytes = used_bytes})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_source = P'.mergeAppend (_source old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
             18 -> Prelude'.fmap
                    (\ !new'Field -> old'Self{_persistence = P'.mergeAppend (_persistence old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
             24 -> Prelude'.fmap (\ !new'Field -> old'Self{_limit_bytes = Prelude'.Just new'Field}) (P'.wireGet 4)
             32 -> Prelude'.fmap (\ !new'Field -> old'Self{_used_bytes = Prelude'.Just new'Field}) (P'.wireGet 4)
...
instance P'.TextMsg DiskStatistics where
  textPut msg
   = do
       ...
       P'.tellT "limit_bytes" (_limit_bytes msg)
       P'.tellT "used_bytes" (_used_bytes msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_source, parse'_persistence, parse'_limit_bytes, parse'_used_bytes]) P'.spaces
        ...
        parse'_limit_bytes
         = P'.try
            (do
               ...
               Prelude'.return (\ o -> o{_limit_bytes = v}))
        parse'_used_bytes
         = P'.try
            (do
               v <- P'.getT "used_bytes"
               Prelude'.return (\ o -> o{_used_bytes = v}))