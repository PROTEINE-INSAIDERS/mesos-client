...
module Mesos.V1.Protos.MountPropagation (MountPropagation(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "MountPropagation"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{mode = mode})
