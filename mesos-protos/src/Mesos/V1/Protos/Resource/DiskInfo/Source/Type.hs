...
instance P'.FromJSON Type where
  parseJSON
   = P'.withText "Mesos.V1.Protos.Resource.DiskInfo.Source.Type.Type"
      (\ msg' ->
        case msg' of
          "UNKNOWN" -> Prelude'.return UNKNOWN
          "PATH" -> Prelude'.return PATH
          "MOUNT" -> Prelude'.return MOUNT
          "BLOCK" -> Prelude'.return BLOCK
          "RAW" -> Prelude'.return RAW
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++
                  " for enum Mesos.V1.Protos.Resource.DiskInfo.Source.Type.Type"))