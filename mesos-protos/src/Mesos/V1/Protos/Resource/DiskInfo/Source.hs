...
data Source = Source{type' :: !(Protos.Resource.DiskInfo.Source.Type), path :: !(P'.Maybe Protos.Resource.DiskInfo.Source.Path),
                     mount :: !(P'.Maybe Protos.Resource.DiskInfo.Source.Mount), vendor :: !(P'.Maybe P'.Utf8),
                     id :: !(P'.Maybe P'.Utf8), metadata :: !(P'.Maybe Protos.Labels), profile :: !(P'.Maybe P'.Utf8)}
   ...
   = P'.withObject "Source"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{type' = type', path = path, mount = mount, vendor = vendor, id = id, metadata = metadata,
                           profile = profile})
        ...
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{mount = P'.mergeAppend (mount old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
             58 -> Prelude'.fmap (\ !new'Field -> old'Self{vendor = Prelude'.Just new'Field}) (P'.wireGet 9)
...
instance P'.TextMsg Source where
  textPut msg
   = do
       ...
       P'.tellT "vendor" (vendor msg)
       P'.tellT "id" (id msg)
       P'.tellT "metadata" (metadata msg)
       P'.tellT "profile" (profile msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'type', parse'path, parse'mount, parse'vendor, parse'id, parse'metadata, parse'profile])
                P'.spaces
