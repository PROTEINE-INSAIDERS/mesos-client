...
module Mesos.V1.Protos.Resource.ReservationInfo (ReservationInfo(..)) where
import Prelude ((+), (/), (++), (.))
...
data ReservationInfo = ReservationInfo{type' :: !(P'.Maybe Protos.Resource.ReservationInfo.Type), role :: !(P'.Maybe P'.Utf8),
                                       principal :: !(P'.Maybe P'.Utf8), labels :: !(P'.Maybe Protos.Labels)}
   ...
   = P'.withObject "ReservationInfo"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{type' = type', role = role, principal = principal, labels = labels})
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{role = Prelude'.Just new'Field}) (P'.wireGet 9)
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{principal = Prelude'.Just new'Field}) (P'.wireGet 9)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{labels = P'.mergeAppend (labels old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg ReservationInfo where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'type', parse'role, parse'principal, parse'labels]) P'.spaces
