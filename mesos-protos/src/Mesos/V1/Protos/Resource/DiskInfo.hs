  ...
  toJSON msg
   = P'.objectNoEmpty
      ([("persistence", P'.toJSON (Prelude'.fmap P'.toJSON (persistence msg))),
        ("volume", P'.toJSON (Prelude'.fmap P'.toJSON (volume msg))), ("source", P'.toJSON (Prelude'.fmap P'.toJSON (source msg)))]
        ++ Prelude'.concat [])

instance P'.FromJSON DiskInfo where
  parseJSON
   = P'.withObject "DiskInfo"
      (\ o ->
        do
