...
instance P'.FromJSON Type where
  parseJSON
   = P'.withText "Mesos.V1.Protos.Resource.ReservationInfo.Type.Type"
      (\ msg' ->
        case msg' of
          "UNKNOWN" -> Prelude'.return UNKNOWN
          "STATIC" -> Prelude'.return STATIC
          "DYNAMIC" -> Prelude'.return DYNAMIC
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++
                  " for enum Mesos.V1.Protos.Resource.ReservationInfo.Type.Type"))