...
module Mesos.V1.Protos.Credential (Credential(..)) where
import Prelude ((+), (/), (++), (.))
...
data Credential = Credential{principal :: !(P'.Utf8), secret :: !(P'.Maybe P'.Utf8)}
   ...
   = P'.withObject "Credential"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{principal = principal, secret = secret})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{principal = new'Field}) (P'.wireGet 9)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{secret = Prelude'.Just new'Field}) (P'.wireGet 9)
...
instance P'.TextMsg Credential where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'principal, parse'secret]) P'.spaces
