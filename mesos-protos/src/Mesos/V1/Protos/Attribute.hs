...
instance P'.ToJSON Attribute where
  toJSON msg
   = P'.objectNoEmpty
      ([("name", P'.toJSON (name msg)), ("type", P'.toJSON (type' msg)),
        ("scalar", P'.toJSON (Prelude'.fmap P'.toJSON (scalar msg))), ("ranges", P'.toJSON (Prelude'.fmap P'.toJSON (ranges msg))),
        ("set", P'.toJSON (Prelude'.fmap P'.toJSON (set msg))), ("text", P'.toJSON (Prelude'.fmap P'.toJSON (text msg)))]
        ++ Prelude'.concat [])

instance P'.FromJSON Attribute where
  parseJSON
   = P'.withObject "Attribute"
      (\ o ->
        do
