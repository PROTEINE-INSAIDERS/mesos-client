...
module Mesos.V1.Protos.Value (Value(..), type', scalar, ranges, set, text) where
import Prelude ((+), (/), (++), (.))
...
data Value = Value{_type' :: !(Protos.Value.Type), _scalar :: !(P'.Maybe Protos.Value.Scalar),
   ...
   = P'.withObject "Value"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_type' = type', _scalar = scalar, _ranges = ranges, _set = set, _text = text})
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_scalar = P'.mergeAppend (_scalar old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{_ranges = P'.mergeAppend (_ranges old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
             34 -> Prelude'.fmap (\ !new'Field -> old'Self{_set = P'.mergeAppend (_set old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg Value where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_type', parse'_scalar, parse'_ranges, parse'_set, parse'_text]) P'.spaces
