...
module Mesos.V1.Protos.ContainerInfo.DockerInfo
       (DockerInfo(..), image, network, port_mappings, privileged, parameters, force_pull_image, volume_driver) where
import Prelude ((+), (/), (++), (.))
...
data DockerInfo = DockerInfo{_image :: !(P'.Utf8), _network :: !(P'.Maybe Protos.ContainerInfo.DockerInfo.Network),
                             _port_mappings :: !(P'.Seq Protos.ContainerInfo.DockerInfo.PortMapping),
                             _privileged :: !(P'.Maybe P'.Bool), _parameters :: !(P'.Seq Protos.Parameter),
                             _force_pull_image :: !(P'.Maybe P'.Bool), _volume_driver :: !(P'.Maybe P'.Utf8)}
                  deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''DockerInfo
   ...
   = P'.withObject "DockerInfo"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{_image = image, _network = network, _port_mappings = port_mappings, _privileged = privileged,
                           _parameters = parameters, _force_pull_image = force_pull_image, _volume_driver = volume_driver})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_image = new'Field}) (P'.wireGet 9)
             16 -> Prelude'.fmap (\ !new'Field -> old'Self{_network = Prelude'.Just new'Field}) (P'.wireGet 14)
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{_port_mappings = P'.append (_port_mappings old'Self) new'Field})
                    (P'.wireGet 11)
             32 -> Prelude'.fmap (\ !new'Field -> old'Self{_privileged = Prelude'.Just new'Field}) (P'.wireGet 8)
             42 -> Prelude'.fmap (\ !new'Field -> old'Self{_parameters = P'.append (_parameters old'Self) new'Field})
                    (P'.wireGet 11)
             48 -> Prelude'.fmap (\ !new'Field -> old'Self{_force_pull_image = Prelude'.Just new'Field}) (P'.wireGet 8)
             58 -> Prelude'.fmap (\ !new'Field -> old'Self{_volume_driver = Prelude'.Just new'Field}) (P'.wireGet 9)
...
instance P'.TextMsg DockerInfo where
  textPut msg
   = do
       ...
       P'.tellT "network" (_network msg)
       P'.tellT "port_mappings" (_port_mappings msg)
       P'.tellT "privileged" (_privileged msg)
       P'.tellT "parameters" (_parameters msg)
       P'.tellT "force_pull_image" (_force_pull_image msg)
       P'.tellT "volume_driver" (_volume_driver msg)
  textGet
   = do
       mods <- P'.sepEndBy
                (P'.choice
                  [parse'_image, parse'_network, parse'_port_mappings, parse'_privileged, parse'_parameters,
                   parse'_force_pull_image, parse'_volume_driver])
                P'.spaces
    ...
    where
        ...
        parse'_privileged
         = P'.try
            (do
               v <- P'.getT "privileged"
               Prelude'.return (\ o -> o{_privileged = v}))
        parse'_parameters
         = P'.try
            (do
               v <- P'.getT "parameters"
               Prelude'.return (\ o -> o{_parameters = P'.append (_parameters o) v}))
        parse'_force_pull_image
         = P'.try
            (do
               v <- P'.getT "force_pull_image"
               Prelude'.return (\ o -> o{_force_pull_image = v}))
        parse'_volume_driver
         = P'.try
            (do
               v <- P'.getT "volume_driver"
               Prelude'.return (\ o -> o{_volume_driver = v}))