...
instance P'.FromJSON Type where
  parseJSON
   = P'.withText "Mesos.V1.Protos.ContainerInfo.Type.Type"
      (\ msg' ->
        case msg' of
          "DOCKER" -> Prelude'.return DOCKER
          "MESOS" -> Prelude'.return MESOS
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++ " for enum Mesos.V1.Protos.ContainerInfo.Type.Type"))