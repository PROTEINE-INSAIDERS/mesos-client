...
module Mesos.V1.Protos.Offer.Operation.Reserve (Reserve(..), resources) where
import Prelude ((+), (/), (++), (.))
...
data Reserve = Reserve{_resources :: !(P'.Seq Protos.Resource)}
               deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Reserve

instance P'.ToJSON Reserve where
  toJSON msg = P'.objectNoEmpty ([("resources", P'.toJSON (Prelude'.fmap P'.toJSON (resources msg)))] ++ Prelude'.concat [])

instance P'.FromJSON Reserve where
  parseJSON
   = P'.withObject "Reserve"
      (\ o ->
        do
