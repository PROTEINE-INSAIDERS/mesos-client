...
module Mesos.V1.Protos.Offer.Operation.DestroyDisk (DestroyDisk(..), source) where
import Prelude ((+), (/), (++), (.))
...
data DestroyDisk = DestroyDisk{_source :: !(Protos.Resource)}
                   deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''DestroyDisk

instance P'.ToJSON DestroyDisk where
  toJSON msg = P'.objectNoEmpty ([("source", P'.toJSON (source msg))] ++ Prelude'.concat [])

instance P'.FromJSON DestroyDisk where
  parseJSON
   = P'.withObject "DestroyDisk"
      (\ o ->
        do
          source <- P'.explicitParseField P'.parseJSON o "source"
          Prelude'.return P'.defaultValue{_source = source})
