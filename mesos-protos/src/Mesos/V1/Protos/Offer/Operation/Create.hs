...
module Mesos.V1.Protos.Offer.Operation.Create (Create(..)) where
import Prelude ((+), (/), (++), (.))
...
data Create = Create{volumes :: !(P'.Seq Protos.Resource)}
   ...
   = P'.withObject "Create"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{volumes = volumes})
