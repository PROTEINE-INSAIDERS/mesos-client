...
module Mesos.V1.Protos.DomainInfo.FaultDomain (FaultDomain(..), region, zone) where
import Prelude ((+), (/), (++), (.))
...
data FaultDomain = FaultDomain{_region :: !(Protos.DomainInfo.FaultDomain.RegionInfo),
                               _zone :: !(Protos.DomainInfo.FaultDomain.ZoneInfo)}
                   deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''FaultDomain
   ...
   = P'.withObject "FaultDomain"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_region = region, _zone = zone})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_region = P'.mergeAppend (_region old'Self) (new'Field)}) (P'.wireGet 11)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_zone = P'.mergeAppend (_zone old'Self) (new'Field)}) (P'.wireGet 11)
...
instance P'.TextMsg FaultDomain where
  textPut msg
   = do
       P'.tellT "region" (_region msg)
       P'.tellT "zone" (_zone msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_region, parse'_zone]) P'.spaces
    ...
    where
        parse'_region
         = P'.try
            (do
               v <- P'.getT "region"
               Prelude'.return (\ o -> o{_region = v}))
        parse'_zone
         = P'.try
            (do
               v <- P'.getT "zone"
               Prelude'.return (\ o -> o{_zone = v}))