...
instance P'.ToJSON RateLimits where
  toJSON msg
   = P'.objectNoEmpty
      ([("limits", P'.toJSON (Prelude'.fmap P'.toJSON (limits msg))),
        ("aggregate_default_qps", P'.toJSON (Prelude'.fmap P'.toJSON (aggregate_default_qps msg))),
        ("aggregate_default_capacity", P'.toJSON (Prelude'.fmap P'.toJSONShowWithPayload (aggregate_default_capacity msg)))]
        ++ Prelude'.concat [])

instance P'.FromJSON RateLimits where
  parseJSON
   = P'.withObject "RateLimits"
      (\ o ->
        do
          limits <- Prelude'.fmap (Prelude'.maybe Prelude'.mempty Prelude'.id)
                     (P'.explicitParseFieldMaybe (Prelude'.mapM P'.parseJSON P'.<=< P'.parseJSON) o "limits")
          aggregate_default_qps <- P'.explicitParseFieldMaybe P'.parseJSON o "aggregate_default_qps"
