...
module Mesos.V1.Protos.HealthCheck.HTTPCheckInfo (HTTPCheckInfo(..)) where
import Prelude ((+), (/), (++), (.))
...
data HTTPCheckInfo = HTTPCheckInfo{protocol :: !(P'.Maybe Protos.NetworkInfo.Protocol), scheme :: !(P'.Maybe P'.Utf8),
                                   port :: !(P'.Word32), path :: !(P'.Maybe P'.Utf8), statuses :: !(P'.Seq P'.Word32)}
   ...
   = P'.withObject "HTTPCheckInfo"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{protocol = protocol, scheme = scheme, port = port, path = path, statuses = statuses})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             40 -> Prelude'.fmap (\ !new'Field -> old'Self{protocol = Prelude'.Just new'Field}) (P'.wireGet 14)
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{scheme = Prelude'.Just new'Field}) (P'.wireGet 9)
             ...
             32 -> Prelude'.fmap (\ !new'Field -> old'Self{statuses = P'.append (statuses old'Self) new'Field}) (P'.wireGet 13)
             34 -> Prelude'.fmap (\ !new'Field -> old'Self{statuses = P'.mergeAppend (statuses old'Self) new'Field})
                    (P'.wireGetPacked 13)
...
instance P'.TextMsg HTTPCheckInfo where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'protocol, parse'scheme, parse'port, parse'path, parse'statuses]) P'.spaces
