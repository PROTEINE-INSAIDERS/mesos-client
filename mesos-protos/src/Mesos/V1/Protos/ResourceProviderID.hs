...
module Mesos.V1.Protos.ResourceProviderID (ResourceProviderID(..), value) where
import Prelude ((+), (/), (++), (.))
...
data ResourceProviderID = ResourceProviderID{_value :: !(P'.Utf8)}
                          deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''ResourceProviderID

instance P'.ToJSON ResourceProviderID where
  toJSON msg = P'.objectNoEmpty ([("value", P'.toJSON (value msg))] ++ Prelude'.concat [])

instance P'.FromJSON ResourceProviderID where
  parseJSON
   = P'.withObject "ResourceProviderID"
      (\ o ->
        do
