...
module Mesos.V1.Protos.CheckInfo.Command (Command(..), command) where
import Prelude ((+), (/), (++), (.))
...
data Command = Command{_command :: !(Protos.CommandInfo)}
   ...
   = P'.withObject "Command"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_command = command})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_command = P'.mergeAppend (_command old'Self) (new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg Command where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_command]) P'.spaces
