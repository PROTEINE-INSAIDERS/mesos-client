...
module Mesos.V1.Protos.CheckInfo.Type (Type(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Type where
  ...
  fromEnum TCP = 3
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.V1.Protos.CheckInfo.Type") . toMaybe'Enum
  succ UNKNOWN = COMMAND
  succ COMMAND = HTTP
  succ HTTP = TCP
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.V1.Protos.CheckInfo.Type"
  pred COMMAND = UNKNOWN
  pred HTTP = COMMAND
  pred TCP = HTTP
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.V1.Protos.CheckInfo.Type"
...
instance P'.ReflectEnum Type where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".mesos.v1.CheckInfo.Type") ["Mesos", "V1"] ["Protos", "CheckInfo"] "Type")
      ["Mesos", "V1", "Protos", "CheckInfo", "Type.hs"]
...
instance P'.FromJSON Type where
  parseJSON
   = P'.withText "Mesos.V1.Protos.CheckInfo.Type.Type"
      (\ msg' ->
        case msg' of
          ...
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++ " for enum Mesos.V1.Protos.CheckInfo.Type.Type"))