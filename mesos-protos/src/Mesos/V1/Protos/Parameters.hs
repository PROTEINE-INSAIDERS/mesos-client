...
module Mesos.V1.Protos.Parameters (Parameters(..), parameter) where
import Prelude ((+), (/), (++), (.))
...
data Parameters = Parameters{_parameter :: !(P'.Seq Protos.Parameter)}
                  deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Parameters
   ...
   = P'.withObject "Parameters"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_parameter = parameter})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_parameter = P'.append (_parameter old'Self) new'Field}) (P'.wireGet 11)
...
instance P'.TextMsg Parameters where
  textPut msg
   = do
       P'.tellT "parameter" (_parameter msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_parameter]) P'.spaces
    ...
    where
        parse'_parameter
         = P'.try
            (do
               v <- P'.getT "parameter"
               Prelude'.return (\ o -> o{_parameter = P'.append (_parameter o) v}))