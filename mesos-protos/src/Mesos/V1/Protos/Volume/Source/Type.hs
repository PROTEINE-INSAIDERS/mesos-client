...
instance P'.FromJSON Type where
  parseJSON
   = P'.withText "Mesos.V1.Protos.Volume.Source.Type.Type"
      (\ msg' ->
        case msg' of
          "UNKNOWN" -> Prelude'.return UNKNOWN
          "DOCKER_VOLUME" -> Prelude'.return DOCKER_VOLUME
          "HOST_PATH" -> Prelude'.return HOST_PATH
          "SANDBOX_PATH" -> Prelude'.return SANDBOX_PATH
          "SECRET" -> Prelude'.return SECRET
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++ " for enum Mesos.V1.Protos.Volume.Source.Type.Type"))