...
module Mesos.V1.Protos.Volume.Source.HostPath (HostPath(..), path, mount_propagation) where
import Prelude ((+), (/), (++), (.))
...
data HostPath = HostPath{_path :: !(P'.Utf8), _mount_propagation :: !(P'.Maybe Protos.MountPropagation)}
                deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''HostPath
   ...
   = P'.withObject "HostPath"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_path = path, _mount_propagation = mount_propagation})
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             18 -> Prelude'.fmap
                    (\ !new'Field ->
                      old'Self{_mount_propagation = P'.mergeAppend (_mount_propagation old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg HostPath where
  textPut msg
   = do
       ...
       P'.tellT "mount_propagation" (_mount_propagation msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_path, parse'_mount_propagation]) P'.spaces
    ...
    where
        ...
        parse'_mount_propagation
         = P'.try
            (do
               v <- P'.getT "mount_propagation"
               Prelude'.return (\ o -> o{_mount_propagation = v}))