...
instance P'.ToJSON SandboxPath where
  toJSON msg
   = P'.objectNoEmpty
      ([("type", P'.toJSON (Prelude'.fmap P'.toJSON (type' msg))), ("path", P'.toJSON (path msg))] ++ Prelude'.concat [])

instance P'.FromJSON SandboxPath where
  parseJSON
   = P'.withObject "SandboxPath"
      (\ o ->
        do
...
instance P'.Default SandboxPath where
  defaultValue = SandboxPath P'.defaultValue P'.defaultValue

instance P'.Wire SandboxPath where
  wireSize ft' self'@(SandboxPath x'1 x'2)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeOpt 1 14 x'1 + P'.wireSizeReq 1 9 x'2)
  wirePutWithSize ft' self'@(SandboxPath x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> put'FieldsSized
       _ -> P'.wirePutErr ft' self'
    where
        put'Fields = P'.sequencePutWithSize [P'.wirePutOptWithSize 8 14 x'1, P'.wirePutReqWithSize 18 9 x'2]
        put'FieldsSized
...
instance P'.GPB SandboxPath

instance P'.ReflectDescriptor SandboxPath where
