...
instance P'.FromJSON Mode where
  parseJSON
   = P'.withText "Mesos.V1.Protos.Volume.Mode.Mode"
      (\ msg' ->
        case msg' of
          "RW" -> Prelude'.return RW
          "RO" -> Prelude'.return RO
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++ " for enum Mesos.V1.Protos.Volume.Mode.Mode"))