...
module Mesos.V1.Protos.ResourceProviderInfo.Storage (Storage(..), plugin) where
import Prelude ((+), (/), (++), (.))
...
data Storage = Storage{_plugin :: !(Protos.CSIPluginInfo)}
               deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Storage

instance P'.ToJSON Storage where
  toJSON msg = P'.objectNoEmpty ([("plugin", P'.toJSON (plugin msg))] ++ Prelude'.concat [])

instance P'.FromJSON Storage where
  parseJSON
   = P'.withObject "Storage"
      (\ o ->
        do
          plugin <- P'.explicitParseField P'.parseJSON o "plugin"
          Prelude'.return P'.defaultValue{_plugin = plugin})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_plugin = P'.mergeAppend (_plugin old'Self) (new'Field)}) (P'.wireGet 11)
...
instance P'.TextMsg Storage where
  textPut msg
   = do
       P'.tellT "plugin" (_plugin msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_plugin]) P'.spaces
