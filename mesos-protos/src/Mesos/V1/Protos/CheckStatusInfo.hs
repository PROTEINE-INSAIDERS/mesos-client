...
module Mesos.V1.Protos.CheckStatusInfo (CheckStatusInfo(..), type', command, http, tcp) where
import Prelude ((+), (/), (++), (.))
...
data CheckStatusInfo = CheckStatusInfo{_type' :: !(P'.Maybe Protos.CheckInfo.Type),
                                       _command :: !(P'.Maybe Protos.CheckStatusInfo.Command),
                                       _http :: !(P'.Maybe Protos.CheckStatusInfo.Http),
                                       _tcp :: !(P'.Maybe Protos.CheckStatusInfo.Tcp)}
                       deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''CheckStatusInfo

instance P'.ToJSON CheckStatusInfo where
  toJSON msg
   = P'.objectNoEmpty
...
instance P'.FromJSON CheckStatusInfo where
  parseJSON
   = P'.withObject "CheckStatusInfo"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_type' = type', _command = command, _http = http, _tcp = tcp})
        ...
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{_http = P'.mergeAppend (_http old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
             34 -> Prelude'.fmap (\ !new'Field -> old'Self{_tcp = P'.mergeAppend (_tcp old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg CheckStatusInfo where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_type', parse'_command, parse'_http, parse'_tcp]) P'.spaces
