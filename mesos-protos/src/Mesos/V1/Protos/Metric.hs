...
module Mesos.V1.Protos.Metric (Metric(..), name, value) where
import Prelude ((+), (/), (++), (.))
...
data Metric = Metric{_name :: !(P'.Utf8), _value :: !(P'.Maybe P'.Double)}
              deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Metric

instance P'.ToJSON Metric where
  toJSON msg
   = P'.objectNoEmpty
      ([("name", P'.toJSON (name msg)), ("value", P'.toJSON (Prelude'.fmap P'.toJSON (value msg)))] ++ Prelude'.concat [])

instance P'.FromJSON Metric where
  parseJSON
   = P'.withObject "Metric"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_name = name, _value = value})
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             17 -> Prelude'.fmap (\ !new'Field -> old'Self{_value = Prelude'.Just new'Field}) (P'.wireGet 1)
...
instance P'.TextMsg Metric where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_name, parse'_value]) P'.spaces
