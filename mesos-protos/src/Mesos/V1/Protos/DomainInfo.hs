...
module Mesos.V1.Protos.DomainInfo (DomainInfo(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "DomainInfo"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{fault_domain = fault_domain})
