...
instance P'.ToJSON Service where
  toJSON msg'
   = P'.String
      (case msg' of
         UNKNOWN -> "UNKNOWN"
...
instance P'.FromJSON Service where
  parseJSON
   = P'.withText "Mesos.V1.Protos.CSIPluginContainerInfo.Service.Service"
      (\ msg' ->
        case msg' of
          "UNKNOWN" -> Prelude'.return UNKNOWN
          "CONTROLLER_SERVICE" -> Prelude'.return CONTROLLER_SERVICE
          "NODE_SERVICE" -> Prelude'.return NODE_SERVICE
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++
                  " for enum Mesos.V1.Protos.CSIPluginContainerInfo.Service.Service"))