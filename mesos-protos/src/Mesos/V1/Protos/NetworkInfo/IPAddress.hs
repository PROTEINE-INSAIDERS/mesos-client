...
module Mesos.V1.Protos.NetworkInfo.IPAddress (IPAddress(..), protocol, ip_address) where
import Prelude ((+), (/), (++), (.))
...
data IPAddress = IPAddress{_protocol :: !(P'.Maybe Protos.NetworkInfo.Protocol), _ip_address :: !(P'.Maybe P'.Utf8)}
                 deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''IPAddress
   ...
   = P'.withObject "IPAddress"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_protocol = protocol, _ip_address = ip_address})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             8 -> Prelude'.fmap (\ !new'Field -> old'Self{_protocol = Prelude'.Just new'Field}) (P'.wireGet 14)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_ip_address = Prelude'.Just new'Field}) (P'.wireGet 9)
...
instance P'.TextMsg IPAddress where
  textPut msg
   = do
       ...
       P'.tellT "ip_address" (_ip_address msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_protocol, parse'_ip_address]) P'.spaces
    ...
    where
        ...
        parse'_ip_address
         = P'.try
            (do
               v <- P'.getT "ip_address"
               Prelude'.return (\ o -> o{_ip_address = v}))