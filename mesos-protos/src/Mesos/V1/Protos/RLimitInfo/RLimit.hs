...
data RLimit = RLimit{type' :: !(P'.Maybe Protos.RLimitInfo.RLimit.Type), hard :: !(P'.Maybe P'.Word64),
                     soft :: !(P'.Maybe P'.Word64)}
   ...
   = P'.withObject "RLimit"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{type' = type', hard = hard, soft = soft})
