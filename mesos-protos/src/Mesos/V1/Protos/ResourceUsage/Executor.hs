...
module Mesos.V1.Protos.ResourceUsage.Executor (Executor(..), executor_info, allocated, statistics, container_id, tasks) where
import Prelude ((+), (/), (++), (.))
...
data Executor = Executor{_executor_info :: !(Protos.ExecutorInfo), _allocated :: !(P'.Seq Protos.Resource),
                         _statistics :: !(P'.Maybe Protos.ResourceStatistics), _container_id :: !(Protos.ContainerID),
                         _tasks :: !(P'.Seq Protos.ResourceUsage.Executor.Task)}
   ...
   = P'.withObject "Executor"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{_executor_info = executor_info, _allocated = allocated, _statistics = statistics,
                           _container_id = container_id, _tasks = tasks})
        ...
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_executor_info = P'.mergeAppend (_executor_info old'Self) (new'Field)})
                    (P'.wireGet 11)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_allocated = P'.append (_allocated old'Self) new'Field}) (P'.wireGet 11)
             26 -> Prelude'.fmap
                    (\ !new'Field -> old'Self{_statistics = P'.mergeAppend (_statistics old'Self) (Prelude'.Just new'Field)})
                    (P'.wireGet 11)
             34 -> Prelude'.fmap (\ !new'Field -> old'Self{_container_id = P'.mergeAppend (_container_id old'Self) (new'Field)})
                    (P'.wireGet 11)
             42 -> Prelude'.fmap (\ !new'Field -> old'Self{_tasks = P'.append (_tasks old'Self) new'Field}) (P'.wireGet 11)
...
instance P'.TextMsg Executor where
  textPut msg
   = do
       ...
       P'.tellT "allocated" (_allocated msg)
       P'.tellT "statistics" (_statistics msg)
       P'.tellT "container_id" (_container_id msg)
       P'.tellT "tasks" (_tasks msg)
  textGet
   = do
       mods <- P'.sepEndBy
                (P'.choice [parse'_executor_info, parse'_allocated, parse'_statistics, parse'_container_id, parse'_tasks])
                P'.spaces
    ...
    where
        ...
        parse'_allocated
         = P'.try
            (do
               v <- P'.getT "allocated"
               Prelude'.return (\ o -> o{_allocated = P'.append (_allocated o) v}))
        parse'_statistics
         = P'.try
            (do
               v <- P'.getT "statistics"
               Prelude'.return (\ o -> o{_statistics = v}))
        parse'_container_id
         = P'.try
            (do
