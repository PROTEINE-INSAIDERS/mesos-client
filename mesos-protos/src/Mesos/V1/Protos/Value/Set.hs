...
module Mesos.V1.Protos.Value.Set (Set(..)) where
import Prelude ((+), (/), (++), (.))
...
data Set = Set{item :: !(P'.Seq P'.Utf8)}
   ...
   = P'.withObject "Set"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{item = item})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{item = P'.append (item old'Self) new'Field}) (P'.wireGet 9)
...
instance P'.TextMsg Set where
  textPut msg
   = do
       P'.tellT "item" (item msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'item]) P'.spaces
    ...
    where
        parse'item
         = P'.try
            (do
               v <- P'.getT "item"
               Prelude'.return (\ o -> o{item = P'.append (item o) v}))