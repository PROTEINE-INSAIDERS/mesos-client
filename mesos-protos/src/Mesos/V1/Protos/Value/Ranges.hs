...
module Mesos.V1.Protos.Value.Ranges (Ranges(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "Ranges"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{range = range})
