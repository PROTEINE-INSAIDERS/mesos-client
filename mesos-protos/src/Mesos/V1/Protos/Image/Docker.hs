...
instance P'.ToJSON Docker where
  toJSON msg
   = P'.objectNoEmpty
      ([("name", P'.toJSON (name msg)), ("credential", P'.toJSON (Prelude'.fmap P'.toJSON (credential msg))),
        ("config", P'.toJSON (Prelude'.fmap P'.toJSON (config msg)))]
        ++ Prelude'.concat [])

instance P'.FromJSON Docker where
  parseJSON
   = P'.withObject "Docker"
      (\ o ->
        do
