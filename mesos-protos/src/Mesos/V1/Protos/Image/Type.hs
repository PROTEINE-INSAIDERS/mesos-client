...
instance P'.FromJSON Type where
  parseJSON
   = P'.withText "Mesos.V1.Protos.Image.Type.Type"
      (\ msg' ->
        case msg' of
          "APPC" -> Prelude'.return APPC
          "DOCKER" -> Prelude'.return DOCKER
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++ " for enum Mesos.V1.Protos.Image.Type.Type"))