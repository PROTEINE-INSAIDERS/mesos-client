...
instance P'.ToJSON Unavailability where
  toJSON msg
   = P'.objectNoEmpty
      ([("start", P'.toJSON (start msg)), ("duration", P'.toJSON (Prelude'.fmap P'.toJSON (duration msg)))] ++ Prelude'.concat [])

instance P'.FromJSON Unavailability where
  parseJSON
   = P'.withObject "Unavailability"
      (\ o ->
        do
