   ...
   = P'.withObject "CheckInfo"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{type' = type', command = command, http = http, tcp = tcp, delay_seconds = delay_seconds,
