...
module Mesos.V1.Protos.VersionInfo (VersionInfo(..), version, build_date, build_time, build_user, git_sha, git_branch, git_tag)
       where
import Prelude ((+), (/), (++), (.))
...
data VersionInfo = VersionInfo{_version :: !(P'.Utf8), _build_date :: !(P'.Maybe P'.Utf8), _build_time :: !(P'.Maybe P'.Double),
                               _build_user :: !(P'.Maybe P'.Utf8), _git_sha :: !(P'.Maybe P'.Utf8),
                               _git_branch :: !(P'.Maybe P'.Utf8), _git_tag :: !(P'.Maybe P'.Utf8)}
                   deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''VersionInfo
   ...
   = P'.withObject "VersionInfo"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{_version = version, _build_date = build_date, _build_time = build_time, _build_user = build_user,
                           _git_sha = git_sha, _git_branch = git_branch, _git_tag = git_tag})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_version = new'Field}) (P'.wireGet 9)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_build_date = Prelude'.Just new'Field}) (P'.wireGet 9)
             25 -> Prelude'.fmap (\ !new'Field -> old'Self{_build_time = Prelude'.Just new'Field}) (P'.wireGet 1)
             34 -> Prelude'.fmap (\ !new'Field -> old'Self{_build_user = Prelude'.Just new'Field}) (P'.wireGet 9)
             42 -> Prelude'.fmap (\ !new'Field -> old'Self{_git_sha = Prelude'.Just new'Field}) (P'.wireGet 9)
             50 -> Prelude'.fmap (\ !new'Field -> old'Self{_git_branch = Prelude'.Just new'Field}) (P'.wireGet 9)
             58 -> Prelude'.fmap (\ !new'Field -> old'Self{_git_tag = Prelude'.Just new'Field}) (P'.wireGet 9)
...
instance P'.TextMsg VersionInfo where
  textPut msg
   = do
       ...
       P'.tellT "build_date" (_build_date msg)
       P'.tellT "build_time" (_build_time msg)
       P'.tellT "build_user" (_build_user msg)
       P'.tellT "git_sha" (_git_sha msg)
       P'.tellT "git_branch" (_git_branch msg)
       P'.tellT "git_tag" (_git_tag msg)
  textGet
   = do
       mods <- P'.sepEndBy
                (P'.choice
                  [parse'_version, parse'_build_date, parse'_build_time, parse'_build_user, parse'_git_sha, parse'_git_branch,
                   parse'_git_tag])
                P'.spaces
    ...
    where
        ...
        parse'_build_date
         = P'.try
            (do
               v <- P'.getT "build_date"
               Prelude'.return (\ o -> o{_build_date = v}))
        parse'_build_time
         = P'.try
            (do
               v <- P'.getT "build_time"
               Prelude'.return (\ o -> o{_build_time = v}))
        parse'_build_user
         = P'.try
            (do
               v <- P'.getT "build_user"
               Prelude'.return (\ o -> o{_build_user = v}))
        parse'_git_sha
         = P'.try
            (do
               v <- P'.getT "git_sha"
               Prelude'.return (\ o -> o{_git_sha = v}))
        parse'_git_branch
         = P'.try
            (do
               v <- P'.getT "git_branch"
               Prelude'.return (\ o -> o{_git_branch = v}))
        parse'_git_tag
         = P'.try
            (do
               v <- P'.getT "git_tag"
               Prelude'.return (\ o -> o{_git_tag = v}))