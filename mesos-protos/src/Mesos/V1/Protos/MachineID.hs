...
module Mesos.V1.Protos.MachineID (MachineID(..), hostname, ip) where
import Prelude ((+), (/), (++), (.))
...
data MachineID = MachineID{_hostname :: !(P'.Maybe P'.Utf8), _ip :: !(P'.Maybe P'.Utf8)}
                 deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''MachineID
   ...
   = P'.withObject "MachineID"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_hostname = hostname, _ip = ip})
...
instance P'.TextMsg MachineID where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_hostname, parse'_ip]) P'.spaces
