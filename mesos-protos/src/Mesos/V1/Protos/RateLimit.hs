...
instance P'.ToJSON RateLimit where
  toJSON msg
   = P'.objectNoEmpty
      ([("qps", P'.toJSON (Prelude'.fmap P'.toJSON (qps msg))), ("principal", P'.toJSON (principal msg)),
        ("capacity", P'.toJSON (Prelude'.fmap P'.toJSONShowWithPayload (capacity msg)))]
        ++ Prelude'.concat [])

instance P'.FromJSON RateLimit where
  parseJSON
   = P'.withObject "RateLimit"
      (\ o ->
        do
