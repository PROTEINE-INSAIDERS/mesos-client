...
instance P'.FromJSON Mode where
  parseJSON
   = P'.withText "Mesos.V1.Protos.MountPropagation.Mode.Mode"
      (\ msg' ->
        case msg' of
          "UNKNOWN" -> Prelude'.return UNKNOWN
          "HOST_TO_CONTAINER" -> Prelude'.return HOST_TO_CONTAINER
          "BIDIRECTIONAL" -> Prelude'.return BIDIRECTIONAL
          _ -> Prelude'.fail
                ("Invalid value " Prelude'.++ Prelude'.show msg' Prelude'.++
                  " for enum Mesos.V1.Protos.MountPropagation.Mode.Mode"))