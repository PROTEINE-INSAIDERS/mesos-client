...
module Mesos.V1.Protos.CgroupInfo.Blkio.Statistics (Statistics(..)) where
import Prelude ((+), (/), (++), (.))
...
data Statistics = Statistics{cfq :: !(P'.Seq Protos.CgroupInfo.Blkio.CFQ.Statistics),
                             cfq_recursive :: !(P'.Seq Protos.CgroupInfo.Blkio.CFQ.Statistics),
                             throttling :: !(P'.Seq Protos.CgroupInfo.Blkio.Throttling.Statistics)}
   ...
   = P'.withObject "Statistics"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{cfq = cfq, cfq_recursive = cfq_recursive, throttling = throttling})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{cfq = P'.append (cfq old'Self) new'Field}) (P'.wireGet 11)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{cfq_recursive = P'.append (cfq_recursive old'Self) new'Field})
                    (P'.wireGet 11)
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{throttling = P'.append (throttling old'Self) new'Field}) (P'.wireGet 11)
...
instance P'.TextMsg Statistics where
  textPut msg
   = do
       P'.tellT "cfq" (cfq msg)
       P'.tellT "cfq_recursive" (cfq_recursive msg)
       P'.tellT "throttling" (throttling msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'cfq, parse'cfq_recursive, parse'throttling]) P'.spaces
    ...
    where
        parse'cfq
         = P'.try
            (do
               v <- P'.getT "cfq"
               Prelude'.return (\ o -> o{cfq = P'.append (cfq o) v}))
        parse'cfq_recursive
         = P'.try
            (do
               v <- P'.getT "cfq_recursive"
               Prelude'.return (\ o -> o{cfq_recursive = P'.append (cfq_recursive o) v}))
        parse'throttling
         = P'.try
            (do
               v <- P'.getT "throttling"
               Prelude'.return (\ o -> o{throttling = P'.append (throttling o) v}))