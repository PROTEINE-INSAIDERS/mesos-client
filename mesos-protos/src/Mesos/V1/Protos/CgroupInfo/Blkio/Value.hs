...
module Mesos.V1.Protos.CgroupInfo.Blkio.Value (Value(..), op, value) where
import Prelude ((+), (/), (++), (.))
...
data Value = Value{_op :: !(P'.Maybe Protos.CgroupInfo.Blkio.Operation), _value :: !(P'.Maybe P'.Word64)}
...
instance P'.ToJSON Value where
  toJSON msg
   = P'.objectNoEmpty
      ([("op", P'.toJSON (Prelude'.fmap P'.toJSON (op msg))),
        ("value", P'.toJSON (Prelude'.fmap P'.toJSONShowWithPayload (value msg)))]
        ++ Prelude'.concat [])
...
instance P'.FromJSON Value where
  parseJSON
   = P'.withObject "Value"
      (\ o ->
        do
          op <- P'.explicitParseFieldMaybe P'.parseJSON o "op"
          value <- P'.explicitParseFieldMaybe (P'.parseJSONReadWithPayload "uint64") o "value"
          Prelude'.return P'.defaultValue{_op = op, _value = value})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             8 -> Prelude'.fmap (\ !new'Field -> old'Self{_op = Prelude'.Just new'Field}) (P'.wireGet 14)
             16 -> Prelude'.fmap (\ !new'Field -> old'Self{_value = Prelude'.Just new'Field}) (P'.wireGet 4)
...
instance P'.TextMsg Value where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_op, parse'_value]) P'.spaces
