...
module Mesos.V1.Protos.Environment (Environment(..), variables) where
import Prelude ((+), (/), (++), (.))
...
data Environment = Environment{_variables :: !(P'.Seq Protos.Environment.Variable)}
                   deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Environment
   ...
   = P'.withObject "Environment"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_variables = variables})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_variables = P'.append (_variables old'Self) new'Field}) (P'.wireGet 11)
...
instance P'.TextMsg Environment where
  textPut msg
   = do
       P'.tellT "variables" (_variables msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_variables]) P'.spaces
    ...
    where
        parse'_variables
         = P'.try
            (do
               v <- P'.getT "variables"
               Prelude'.return (\ o -> o{_variables = P'.append (_variables o) v}))