...
module Mesos.V1.Maintenance.Protos.ClusterStatus.DrainingMachine (DrainingMachine(..), id, statuses) where
import Prelude ((+), (/), (++), (.))
...
data DrainingMachine = DrainingMachine{_id :: !(Protos.MachineID), _statuses :: !(P'.Seq Protos.InverseOfferStatus)}
                       deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''DrainingMachine

instance P'.ToJSON DrainingMachine where
  toJSON msg
   = P'.objectNoEmpty
      ([("id", P'.toJSON (id msg)), ("statuses", P'.toJSON (Prelude'.fmap P'.toJSON (statuses msg)))] ++ Prelude'.concat [])

instance P'.FromJSON DrainingMachine where
  parseJSON
   = P'.withObject "DrainingMachine"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_id = id, _statuses = statuses})
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_statuses = P'.append (_statuses old'Self) new'Field}) (P'.wireGet 11)
...
instance P'.TextMsg DrainingMachine where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_id, parse'_statuses]) P'.spaces
