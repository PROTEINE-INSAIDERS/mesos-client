...
module Mesos.V1.Maintenance.Protos.ClusterStatus (ClusterStatus(..), draining_machines, down_machines) where
import Prelude ((+), (/), (++), (.))
...
data ClusterStatus = ClusterStatus{_draining_machines :: !(P'.Seq Protos.ClusterStatus.DrainingMachine),
                                   _down_machines :: !(P'.Seq Protos.MachineID)}
                     deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''ClusterStatus
   ...
   = P'.withObject "ClusterStatus"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_draining_machines = draining_machines, _down_machines = down_machines})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_draining_machines = P'.append (_draining_machines old'Self) new'Field})
                    (P'.wireGet 11)
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_down_machines = P'.append (_down_machines old'Self) new'Field})
                    (P'.wireGet 11)
...
instance P'.TextMsg ClusterStatus where
  textPut msg
   = do
       P'.tellT "draining_machines" (_draining_machines msg)
       P'.tellT "down_machines" (_down_machines msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_draining_machines, parse'_down_machines]) P'.spaces
       Prelude'.return (Prelude'.foldl (\ v f -> f v) P'.defaultValue mods)
    where
        parse'_draining_machines
         = P'.try
            (do
               v <- P'.getT "draining_machines"
               Prelude'.return (\ o -> o{_draining_machines = P'.append (_draining_machines o) v}))
        parse'_down_machines
         = P'.try
            (do
               v <- P'.getT "down_machines"
               Prelude'.return (\ o -> o{_down_machines = P'.append (_down_machines o) v}))