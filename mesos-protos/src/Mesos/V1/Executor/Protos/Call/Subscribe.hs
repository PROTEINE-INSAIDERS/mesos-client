...
module Mesos.V1.Executor.Protos.Call.Subscribe (Subscribe(..), unacknowledged_tasks, unacknowledged_updates) where
import Prelude ((+), (/), (++), (.))
...
data Subscribe = Subscribe{_unacknowledged_tasks :: !(P'.Seq Protos.TaskInfo),
                           _unacknowledged_updates :: !(P'.Seq Protos.Call.Update)}
   ...
   = P'.withObject "Subscribe"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{_unacknowledged_tasks = unacknowledged_tasks, _unacknowledged_updates = unacknowledged_updates})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap
                    (\ !new'Field -> old'Self{_unacknowledged_tasks = P'.append (_unacknowledged_tasks old'Self) new'Field})
                    (P'.wireGet 11)
             18 -> Prelude'.fmap
                    (\ !new'Field -> old'Self{_unacknowledged_updates = P'.append (_unacknowledged_updates old'Self) new'Field})
                    (P'.wireGet 11)
...
instance P'.TextMsg Subscribe where
  textPut msg
   = do
       P'.tellT "unacknowledged_tasks" (_unacknowledged_tasks msg)
       P'.tellT "unacknowledged_updates" (_unacknowledged_updates msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_unacknowledged_tasks, parse'_unacknowledged_updates]) P'.spaces
       Prelude'.return (Prelude'.foldl (\ v f -> f v) P'.defaultValue mods)
    where
        parse'_unacknowledged_tasks
         = P'.try
            (do
               v <- P'.getT "unacknowledged_tasks"
               Prelude'.return (\ o -> o{_unacknowledged_tasks = P'.append (_unacknowledged_tasks o) v}))
        parse'_unacknowledged_updates
         = P'.try
            (do
               v <- P'.getT "unacknowledged_updates"
               Prelude'.return (\ o -> o{_unacknowledged_updates = P'.append (_unacknowledged_updates o) v}))