...
module Mesos.V1.Executor.Protos.Event.Kill (Kill(..), task_id, kill_policy) where
import Prelude ((+), (/), (++), (.))
...
data Kill = Kill{_task_id :: !(Protos.TaskID), _kill_policy :: !(P'.Maybe Protos.KillPolicy)}
...
instance P'.ToJSON Kill where
  toJSON msg
   = P'.objectNoEmpty
      ([("task_id", P'.toJSON (task_id msg)), ("kill_policy", P'.toJSON (Prelude'.fmap P'.toJSON (kill_policy msg)))] ++
        Prelude'.concat [])
   ...
   = P'.withObject "Kill"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_task_id = task_id, _kill_policy = kill_policy})
...
instance P'.TextMsg Kill where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_task_id, parse'_kill_policy]) P'.spaces
