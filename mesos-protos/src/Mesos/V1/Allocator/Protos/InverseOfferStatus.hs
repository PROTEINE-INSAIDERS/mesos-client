...
module Mesos.V1.Allocator.Protos.InverseOfferStatus (InverseOfferStatus(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "InverseOfferStatus"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{status = status, framework_id = framework_id, timestamp = timestamp})
