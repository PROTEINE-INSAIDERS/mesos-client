...
module Mesos.V1.Scheduler.Protos.Response.ReconcileOperations (ReconcileOperations(..)) where
import Prelude ((+), (/), (++), (.))
        ...
        do
          operation_statuses <- Prelude'.fmap (Prelude'.maybe Prelude'.mempty Prelude'.id)
                                 (P'.explicitParseFieldMaybe (Prelude'.mapM P'.parseJSON P'.<=< P'.parseJSON) o
                                   "operation_statuses")
          Prelude'.return P'.defaultValue{operation_statuses = operation_statuses})
