...
module Mesos.V1.Scheduler.Protos.Event.RescindInverseOffer (RescindInverseOffer(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "RescindInverseOffer"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{inverse_offer_id = inverse_offer_id})
