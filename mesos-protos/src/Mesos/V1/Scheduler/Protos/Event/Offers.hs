...
module Mesos.V1.Scheduler.Protos.Event.Offers (Offers(..), offers) where
import Prelude ((+), (/), (++), (.))
...
data Offers = Offers{_offers :: !(P'.Seq Protos.Offer)}
              deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Offers
   ...
   = P'.withObject "Offers"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_offers = offers})
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{_offers = P'.append (_offers old'Self) new'Field}) (P'.wireGet 11)
...
instance P'.TextMsg Offers where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_offers]) P'.spaces
