...
module Mesos.V1.Scheduler.Protos.Event.Subscribed (Subscribed(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "Subscribed"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{framework_id = framework_id, heartbeat_interval_seconds = heartbeat_interval_seconds,
