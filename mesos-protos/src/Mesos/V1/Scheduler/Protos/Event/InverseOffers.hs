...
instance P'.ToJSON InverseOffers where
  toJSON msg
   = P'.objectNoEmpty ([("inverse_offers", P'.toJSON (Prelude'.fmap P'.toJSON (inverse_offers msg)))] ++ Prelude'.concat [])

instance P'.FromJSON InverseOffers where
  parseJSON
   = P'.withObject "InverseOffers"
      (\ o ->
        do
