...
module Mesos.V1.Scheduler.Protos.Event.UpdateOperationStatus (UpdateOperationStatus(..), status) where
import Prelude ((+), (/), (++), (.))
...
data UpdateOperationStatus = UpdateOperationStatus{_status :: !(Protos.OperationStatus)}
                             deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''UpdateOperationStatus

instance P'.ToJSON UpdateOperationStatus where
  toJSON msg = P'.objectNoEmpty ([("status", P'.toJSON (status msg))] ++ Prelude'.concat [])

instance P'.FromJSON UpdateOperationStatus where
  parseJSON
   = P'.withObject "UpdateOperationStatus"
      (\ o ->
        do
