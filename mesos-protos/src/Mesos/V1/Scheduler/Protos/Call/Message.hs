...
module Mesos.V1.Scheduler.Protos.Call.Message (Message(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "Message"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{agent_id = agent_id, executor_id = executor_id, data' = data'})
