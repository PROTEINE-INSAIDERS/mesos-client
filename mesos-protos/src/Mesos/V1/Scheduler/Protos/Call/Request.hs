...
module Mesos.V1.Scheduler.Protos.Call.Request (Request(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "Request"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{requests = requests})
