...
module Mesos.V1.Scheduler.Protos.Call.Shutdown (Shutdown(..), executor_id, agent_id) where
import Prelude ((+), (/), (++), (.))
...
data Shutdown = Shutdown{_executor_id :: !(Protos.ExecutorID), _agent_id :: !(Protos.AgentID)}
                deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Shutdown
   ...
   = P'.withObject "Shutdown"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_executor_id = executor_id, _agent_id = agent_id})
...
instance P'.TextMsg Shutdown where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_executor_id, parse'_agent_id]) P'.spaces
