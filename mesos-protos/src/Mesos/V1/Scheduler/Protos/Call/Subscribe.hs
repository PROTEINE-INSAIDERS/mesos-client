...
module Mesos.V1.Scheduler.Protos.Call.Subscribe (Subscribe(..)) where
import Prelude ((+), (/), (++), (.))
   ...
   = P'.withObject "Subscribe"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{framework_info = framework_info, suppressed_roles = suppressed_roles})
