...
module Mesos.V1.Scheduler.Protos.Call.AcknowledgeOperationStatus
       (AcknowledgeOperationStatus(..), agent_id, resource_provider_id, uuid, operation_id) where
import Prelude ((+), (/), (++), (.))
...
data AcknowledgeOperationStatus = AcknowledgeOperationStatus{_agent_id :: !(P'.Maybe Protos.AgentID),
                                                             _resource_provider_id :: !(P'.Maybe Protos.ResourceProviderID),
                                                             _uuid :: !(P'.ByteString), _operation_id :: !(Protos.OperationID)}
                                  deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data,
                                            Prelude'.Generic)

Control.Lens.TH.makeLenses ''AcknowledgeOperationStatus
   ...
   = P'.withObject "AcknowledgeOperationStatus"
      (\ o ->
        do
          ...
          Prelude'.return
           P'.defaultValue{_agent_id = agent_id, _resource_provider_id = resource_provider_id, _uuid = uuid,
        ...
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             18 -> Prelude'.fmap
                    ...
                    (P'.wireGet 11)
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{_uuid = new'Field}) (P'.wireGet 12)
             34 -> Prelude'.fmap (\ !new'Field -> old'Self{_operation_id = P'.mergeAppend (_operation_id old'Self) (new'Field)})
                    (P'.wireGet 11)
...
instance P'.TextMsg AcknowledgeOperationStatus where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_agent_id, parse'_resource_provider_id, parse'_uuid, parse'_operation_id]) P'.spaces
