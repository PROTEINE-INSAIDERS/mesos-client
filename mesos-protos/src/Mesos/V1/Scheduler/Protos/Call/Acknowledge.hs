...
module Mesos.V1.Scheduler.Protos.Call.Acknowledge (Acknowledge(..), agent_id, task_id, uuid) where
import Prelude ((+), (/), (++), (.))
...
data Acknowledge = Acknowledge{_agent_id :: !(Protos.AgentID), _task_id :: !(Protos.TaskID), _uuid :: !(P'.ByteString)}
                   deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Acknowledge
   ...
   = P'.withObject "Acknowledge"
      (\ o ->
        do
          ...
          Prelude'.return P'.defaultValue{_agent_id = agent_id, _task_id = task_id, _uuid = uuid})
...
instance P'.TextMsg Acknowledge where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_agent_id, parse'_task_id, parse'_uuid]) P'.spaces
